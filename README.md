...
2. LangChain loads and splits each PDF once. OpenAI embeddings are indexed in Pinecone, with a separate namespace for each owner. A SQLite catalog records document ownership and ingestion status.
...
| `PINECONE_API_KEY`            | Access to your existing Pinecone index.                                         |
...
The setup script preserves existing configuration and never prints secrets. For local migration only, the agent can reuse `OPENAI_API_KEY` and `SERPAPI_KEY` from the original `server/.env` when those entries are empty in `.env.local`. A clean clone needs its own keys. Missing configuration returns a setup error; there is no mock or in-memory retrieval fallback, and the application never provisions a Pinecone index automatically.
...
Every document read, retrieval, and deletion checks the server-owned SQLite catalog. Pinecone queries use an owner-derived namespace plus the selected document IDs. Neither the model nor the browser chooses an owner or namespace. The Python service requires the internal service token. CopilotKit run, reconnect, status, and stop operations also namespace their shared memory keys by owner, and its unscoped inspector/history endpoints are disabled.
...
Tests cover PDF ingestion, persistent ownership, cross-user access rejection, selected multi-document retrieval, bounded retries, actual ADK/AG-UI event conversion, cancellation, signed sessions, runtime isolation, and browser interactions. Automated tests use synthetic documents and fake external providers; they do not call paid APIs. Live Pinecone/OpenAI/SerpAPI verification additionally requires working credentials and an existing index.
